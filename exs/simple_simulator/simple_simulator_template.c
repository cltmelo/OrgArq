...
int line_process(char* linha); 

// Funcao que separa somente o pedaco de interesse do IR;
int pega_pedaco(int ir, int a, int b); 

// Rotate Left 16 bits
unsigned int _rotLeft(const unsigned int value, int shift);

// Rotate Right 16 bits
unsigned int _rotRight(const unsigned int value, int shift);
					...
					{   case 0: reg[rx] = reg[rx] << pega_pedaco(IR,3,0);           break;
						...
						default:
								if(pega_pedaco(IR,6,5)==2) // ROTATE LEFT
									reg[rx] = _rotLeft(reg[rx],pega_pedaco(IR,3,0));
								else
									reg[rx] = _rotRight(reg[rx],pega_pedaco(IR,3,0)); 
								break;
					}
